...
function validateTicketEligibility() {
  ...
  if (selectedTickets.child > 0) {
    if (!birthDate) {
      ...
      if (age >= 13) {
        errors.push(
          "Child tickets are only valid for visitors under 13 years old."
        );
      }
    }
  }
  ...
  if (selectedTickets.senior > 0) {
    if (!birthDate) {
      ...
      if (age < 65) {
        errors.push(
          "Senior tickets are only valid for visitors 65 years and older."
        );
      }
    }
  }
...
function attachEventListeners() {
  ...
  document.querySelectorAll(".select-ticket").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      showNotification(
        `${type.charAt(0).toUpperCase() + type.slice(1)} ticket added!`,
        "success"
      );
    });
  });
...
function updateTicketSelection() {
  ...
  container.querySelectorAll(".discard-ticket").forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      toggleAdditionalFields();
      showNotification(
        `${type.charAt(0).toUpperCase() + type.slice(1)} tickets removed`,
        "info"
      );
    });
  });
    ...
    .map(
      (b, i) => `
        <tr>
            ...
            <td>
                <button onclick='openTicketModal(${JSON.stringify(b).replace(
                  /'/g,
                  "&#39;"
                )})'>
...
function downloadTicket() {
  ...
  y += 7;
  pdf.text(
    `Payment: ${document.getElementById("conf-payment-method").textContent}`,
    20,
    y
  );
  y += 12;
  ...
  qrImg.onload = function () {
    ...
    pdf.text("Please bring valid ID and this booking reference.", 105, 285, {
      align: "center",
    });
...
// ==================== CONSOLE LOG ====================
console.log(
  "%c🎫 Zoo Planet Ticket System Ready!",
