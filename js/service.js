...
const EVENTS_API = "https://zoo-planet-backend.onrender.com";
const TOURS_API = "https://zoo-planet-backend.onrender.com";
...
function updatePaymentSummary(baseAmount, prefix) {
  ...
  console.log(
    `💰 Payment Summary Updated [${
      prefix || "membership"
    }]: Base: ₹${baseAmount}, GST (18%): ₹${tax}, Total: ₹${total}`
  );
...
function openMembershipModal(plan) {
  ...
  if (familySection) {
    familySection.style.display =
      plan === "family" || plan === "premium" ? "block" : "none";
  }
...
function openEventModal(eventKey) {
  ...
  console.log(
    `🎉 Opening Event: ${selectedEventType} (${eventKey}) - ₹${eventAmount}`
  );
...
function openTourModal(tourKey) {
  ...
  console.log(
    `🦁 Opening Tour: ${selectedTourType} (${tourKey}) - ₹${tourPricePerPerson}/person`
  );
...
document.querySelectorAll(".service-nav-btn").forEach((btn) => {
  btn.addEventListener("click", function (e) {
    ...
    document
      .querySelectorAll(".service-nav-btn")
...
document.querySelectorAll(".season-tab").forEach((tab) => {
  tab.addEventListener("click", function () {
    ...
    document
      .querySelectorAll(".season-content")
...
function setupQuantityUpdates() {
  // Event guests update
  const eventGuestsInputs = document.querySelectorAll(
    '#eventForm input[name="guests"]'
  );
  ...
  const tourParticipantsInputs = document.querySelectorAll(
    '#tourForm input[name="participants"]'
  );
...
if (membershipForm) {
  membershipForm.addEventListener("submit", async function (e) {
    ...
    const paymentMethod = this.querySelector(
    ...
    const termsCheckbox = this.querySelector(
    ...
    try {
      ...
      const res = await fetch(`${MEMBERSHIP_API}/api/memberships`, {
        method: "POST",
      ...
      alert(
        `✅ Payment Successful!\n\nMembership: ${selectedMembershipPlan}\nAmount Paid: ₹${finalAmount.toFixed(
          2
        )}\nReference: ${
          data.membershipId || "Confirmed"
...
if (eventForm) {
  eventForm.addEventListener("submit", async function (e) {
    ...
    // Check payment method
    const paymentMethod = eventForm.querySelector(
      'input[name="paymentMethod"]:checked'
    );
    ...
    const termsCheckbox = eventForm.querySelector(
      'input[type="checkbox"][name="terms"]'
    );
    if (!termsCheckbox || !termsCheckbox.checked) {
      alert("❌ You must accept Terms & Conditions");
    ...
    // Get all form values
    const guests =
      parseInt(eventForm.querySelector('input[name="guests"]')?.value) || 1;
    ...
    const formData = {
      ...
      specialRequests:
        eventForm.querySelector('textarea[name="specialRequests"]')?.value ||
        "",
    ...
    try {
      ...
      alert(
        `✅ Payment Successful!\n\nEvent: ${selectedEventType}\nGuests: ${guests}\nAmount Paid: ₹${finalAmount.toFixed(
          ...
          data.booking?.bookingReference || "Confirmed"
        }\n\nConfirmation sent to your email!`
      );
      eventForm.reset();
    } catch (err) {
...
if (tourForm) {
  tourForm.addEventListener("submit", async function (e) {
    ...
    const participants =
      parseInt(this.querySelector('input[name="participants"]')?.value) || 1;
    ...
    try {
      ...
      const res = await fetch(`${TOURS_API}/api/tours`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
...
document
  .querySelectorAll(
    ".membership-card, .event-card, .tour-card, .hours-card, .accessibility-card"
  )
  .forEach((card) => {
...
// ==================== CONSOLE LOG ====================
console.log(
  "%c🦁 Zoo Planet Services Loaded!",
  "color: #2d8659; font-size: 16px; font-weight: bold;"
);
console.log(
  "%c💳 Payment System Active",
  "color: #2d8659; font-size: 14px; font-weight: bold;"
);
console.log(
  "%c✅ Features: Pre-set prices | GST calculation | Payment methods | Real-time updates",
  "color: #666;"
);
